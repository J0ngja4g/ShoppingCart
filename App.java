...
public class App{
     ...
     */
    private static void checkpass(String testName, boolean condition) {
        if (condition) {
    ...
    public static void main(String[] args) {
        ...
        checkpass("New cart should be empty", cart.getItemCount() == 0 && cart.getTotalPrice() == 0.0);

        cart.addItem("P001", 3); // Apple x3
        checkpass("Add new item correctly", cart.getItemCount() == 1 && cart.getTotalPrice() == 20.0); // BOGO: pay for 2

        cart.addItem("P001", 2); // Apple x3+2=5
        checkpass("Add existing item increases quantity", cart.getItemCount() == 1 && cart.getTotalPrice() == 30.0); // BOGO: pay for 3

        cart.addItem("P002", 5); // Soda x5
        checkpass("Add another new item", cart.getItemCount() == 2 && cart.getTotalPrice() == 30.0 + 25.0); // 55.0

        cart.addItem("P002", 1); // Soda x5+1=6
        checkpass("Add item to meet bulk discount threshold", cart.getItemCount() == 2 && cart.getTotalPrice() == 30.0 + (30.0 * 0.9)); // 30 + 27 = 57.0

        cart.addItem("P999", 1); // Non-existent product
        checkpass("Adding non-existent product does not change cart", cart.getItemCount() == 2 && cart.getTotalPrice() == 57.0);

        cart.addItem("P003", 0); // Invalid quantity
        checkpass("Adding item with zero quantity does not change cart", cart.getItemCount() == 2 && cart.getTotalPrice() == 57.0);

        cart.removeItem("P002"); // Remove soda
        checkpass("Remove item correctly updates count and price", cart.getItemCount() == 1 && cart.getTotalPrice() == 30.0);

        cart.removeItem("P999"); // Remove non-existent item
        checkpass("Removing non-existent item does not change cart", cart.getItemCount() == 1 && cart.getTotalPrice() == 30.0);
        
        cart.clearCart();
        checkpass("Clear cart works correctly", cart.getItemCount() == 0 && cart.getTotalPrice() == 0.0);
        ...
        checkpass("BOGO Strategy (1 item)", pricingService.calculateItemPrice(singleApple) == 10.0);
        checkpass("BOGO Strategy (2 items)", pricingService.calculateItemPrice(twoApples) == 10.0);
        checkpass("Bulk Strategy (below threshold)", pricingService.calculateItemPrice(fiveSodas) == 25.0);
        checkpass("Bulk Strategy (above threshold)", pricingService.calculateItemPrice(tenSodas) == 45.0);
        checkpass("Default Strategy", pricingService.calculateItemPrice(normalBread) == 60.0);
